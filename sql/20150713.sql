...
declare
  ...
  _accession integer;
  _oldstatus varchar(20);
  _newstatus varchar(20);
BEGIN
  IF NEW.idstatus < OLD.idstatus THEN
    _oldstatus = (select status from status where idstatus=OLD.idstatus);
    _newstatus = (select status from status where idstatus=NEW.idstatus);
    raise 'No está permitido cambiar de estado % a %', _oldstatus, _newstatus;
  END IF;
