...
/* inserta datos de paciente */
  ...
  if (_idprocedure is null) then
    ...
    update procedure set procedure=_prestacionnombre where codprocedure=_prestacioncod::varchar(100);
  end if;

  /* inserta datos de turno */
  _idstudy = (select idstudy from study where accessionnumber = _accession);
  if ( _idstudy is null) then
    insert into study(idpatient, idrequestingphysician, idperformingphysician, accessionnumber,  studydate, modality, idsucursal)
      values(_idpatient, _idprofder, _idprofefe, _accession, _fecha_desde, _modalidad, _id_sucursal);
    ...
    if (_idstudyprocedure is null) then
      insert into studyprocedure(idstudy, idprocedure, qty, idstatus) values(_idstudy, _idprocedure, _prestacionqty, 1);
    end if;
  end if;
end
$BODY$
  ...
  COST 100;
ALTER FUNCTION insert_study(character varying, timestamp without time zone, character varying, character varying, character varying, character varying, timestamp without time zone, timestamp without time zone, character varying, date, character varying, character varying, character varying, sex_type, character varying, character varying, character varying, character varying, character varying, integer, integer, character varying, character varying, integer, integer)
  OWNER TO postgres;
